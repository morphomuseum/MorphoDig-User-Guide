...
3D Curves (series of 3D cubic Bezier curves) are constructed in MorphoDig using 2 series of landmarks : a series of N ``curve node" landmarks, and a series of N ``curve handle".  Curve nodes and handles can be set on surfaces by pressing ``L" + left mouse click.\\

 In the ``curve nodes" landmark mode (button \includegraphics[scale=0.7]{images/04/curve_nodes.png} active), pressing ``L" + left mouse click results in the creation a ``curve node" landmark (a dark green for a curve starting node, and then light red ones). In this mode (``curve nodes" landmark mode = \includegraphics[scale=0.7]{images/04/curve_nodes.png} active), pressing ``Shift" + left mouse click results in the creation a new starting ``curve node" landmark; pressing ``J" + left mouse click results in the creation a new milestone ``curve node" landmark; pressing ``K" + left mouse click results in the creation a new ``curve node" landmark that will be connected to the preceding starting node to close the curve; \\

 In the ``curve handles" landmark mode (button \includegraphics[scale=0.7]{images/04/curve_handles.png} active), pressing ``L" + left mouse click will create a ``target" landmark (a violet transparent one).
...
- by setting curve node "milestones", in order to create \textbf{contiguous} curve segments (see section \ref{section_curve_segment2} p.\pageref{section_curve_segment2}, and a practical example Fig. \ref{curve_milestone} p. \pageref{curve_milestone}). Note that when exporting curves as equidistant semi-landmarks, it is possible to avoid curve milestone duplication. 
\\
 You may also decide to save normal and target landmarks information inside the  output. This is useful when you need to use both type 1 landmarks (normal and target landmarks) and semi-landmards (3D curves) in Geometric Morphometric analyses. 

In the example presented Fig. \ref{export_curves} p. \pageref{export_curves}, 4 segments have been digitized, and the default number of landmarks per segment is 60, but the user has chosen to export segments using 25, 20, 5 and 20 equidistant landmarks instead.
...
\begin{figure}
 ...
 \captionof{figure}{Export curves as landmark file window. Here, 5 curve segments have been digitized, and the default number of landmarks per segment is 60, but the user has chosen to export segments 2 and 4 using 20 equidistant landmarks instead.}
 \label{export_curves}
 \end{figure}
...
This option opens the "Open raw volume" Dialog (see Fig. \ref{open_raw} p.\pageref{open_raw}).

\begin{figure}
  \centering  
 \includegraphics[scale=1]{images/07/volume/open_raw2.png}
 \captionof{figure}{Open raw volume dialog. In order to open a raw volume, information is required. \textbf{1)} Data type (8 bits, 16 bits signed or unsigned, 32 bits or 64 bits). \textbf{2)} Data dimensions in X, Y and Z. \textbf{3)} Header Size. \textbf{4)} Given data type, data dimensions and header size, requested size in bytes must be equal to filesize. \textbf{5)} Data endianess: little or big endian. \textbf{6)} Index order: slices either are back-to-front or front-to-back ordered.  Most CT datasets are back-to-front ordered: the first image image is furthest from the viewer, and subsequent images move towards the viewer. If you do not choose the right option, a mirror image of your volume will appear on the screen (which can be corrected later on). \textbf{7)} voxel size in x, y and z dimensions (most ofen expressed in mm)}
\label{open_raw}
\end{figure}

\subsection{Open 3D tiff file}
This option opens the "Open 3D Tiff file or 2D Tiff /BMP/PNG stack " dialog (see Fig. \ref{open_tiff} p.\pageref{open_tiff}). Data type and dimensions are automatically detected, but you have to fill index order and voxel size.

\begin{figure}
  \centering  
 \includegraphics[scale=1]{images/07/volume/open_tiff2.png}
 \captionof{figure}{Open 3D Tiff file or 2D Tiff/BMP/PNG stack dialog \textbf{1)} Index order: slices either are back-to-front or front-to-back ordered.  Most CT datasets are back-to-front ordered: the first image image is furthest from the viewer, and subsequent images move towards the viewer. If you do not choose the right option, a mirror image of your volume will appear on the screen (which can be corrected later on). \textbf{2)} voxel size in x, y and z dimensions (most ofen expressed in mm)}
\label{open_tiff}
\end{figure}
...
Once a number of .tiff files has been selected, the "Open 3D Tiff file or 2D Tiff stack " dialog (see Fig. \ref{open_tiff} p.\pageref{open_tiff}) will also be opened. Data type and dimensions are automatically detected, but you have to fill index order and voxel size.


\subsection{Open DICOM 2D stack}
Once a number of .dcm or .ima files has been selected, the "Open 3D Tiff file or 2D Tiff stack " dialog (see Fig. \ref{open_dcm} p.\pageref{open_dcm}) will also be opened. Data type, data dimensions and voxel size are automatically detected, but you have to fill index order.


\begin{figure}
  \centering  
 \includegraphics[scale=0.5]{images/07/volume/open_dcm.png}
 \captionof{figure}{Open dicom stack. Index order: slices either are back-to-front or front-to-back ordered.  Most CT datasets are back-to-front ordered: the first image image is furthest from the viewer, and subsequent images move towards the viewer. If you do not choose the right option, a mirror image of your volume will appear on the screen (which can be corrected later on).}
\label{open_dcm}
\end{figure}

\subsection{Open BMP image stack}

Once a number of 8bit .bmp files has been selected, the "Open 3D Tiff file or 2D Tiff/BMP/PNG stack " dialog (see Fig. \ref{open_tiff} p.\pageref{open_tiff}) will also be opened. Data type and dimensions are automatically detected, but you have to fill index order and voxel size. Warning: RGB (24 bits) bmp files can not currently be read.

\subsection{Open PNG image stack}

Once a number of 8bit .png files has been selected, the "Open 3D Tiff file or 2D Tiff/BMP/PNG stack " dialog (see Fig. \ref{open_tiff} p.\pageref{open_tiff}) will also be opened. Data type and dimensions are automatically detected, but you have to fill index order and voxel size. Warning: RGB (24 bits) png files can not currently be read.
...
A volume must be selected in order to use this option. This menu opens the "Save MHD/MHA" dialog (see Fig. \ref{save_mhd_mha} p.\pageref{save_mhd_mha}). MHD/MHA files can be compressed. 
\begin{figure}
  \centering  
 \includegraphics[scale=0.5]{images/07/volume/save_mhd_mha.png}
 \captionof{figure}{Save volume as MHD/MHA.}
\label{save_mhd_mha}
\end{figure}
...
A volume must be selected in order to use this option. This menu opens the "Save VTI" dialog (see Fig. \ref{save_vti} p.\pageref{save_vti}). VTI files can be compressed using different methods, and can be saved either using a little endian or big endian byte order. 
\begin{figure}
  \centering  
 \includegraphics[scale=0.5]{images/07/volume/save_vti.png}
 \captionof{figure}{Save volume as VTI.}
\label{save_vti}
\end{figure}
