...
\section{Create 3D printing support surfaces}
When planning to 3D print a 3D models composed of non connex elements (for instance : the cranium and the mandible of a single individual, or the skeleton of the fetus of a vertebrate specimen), you may need to manually place in 3D space surface elements linking those different parts. Such elements are referred to as "3D printing support" elements, and can ba created via the "Create 3D printing support material (cylinders)" dialog (Fig. \ref{cylinders_dialog} p.\pageref{cylinders_dialog}). Different options are available, and in particular, the cylinders can be given either conic of flat ends (Fig. \ref{conic_non_conic} p.\pageref{conic_non_conic}). A practical examples is shown in Fig. \ref{support_example} p.\pageref{support_example}.
\begin{figure}
  \centering
  \includegraphics[scale=0.5]{images/09/create_3D_printing_support_surfaces/cylinders_dialog.png} 
	\caption{Create cylinders dialog. Different options are available. Several cylinders sharing the same properties can be created at once. The shaft lenght and radius can be defined, as well as the resolution (number of sides). The bottom and top sides of the cylinders can be either flat, or conic.}
...
\begin{figure}
  ...
  \includegraphics[scale=0.5]{images/09/create_3D_printing_support_surfaces/conic_non_conic.png} 
	\caption{Different cylinder shapes. \textbf{Left}: cylinders can be given conic bottom and top ends. \textbf{Right:} cylinders can have flat ends.}
 \label{conic_non_conic}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[scale=0.40]{images/09/create_3D_printing_support_surfaces/support_example.png} 
	\caption{Creating cylinders to prepare the 3D printing of the skeleton of a newborn textit{Lemur catta}. \textbf{Left:} The skeleton of this neonate specimen contains many non connex elements: many bones are not fully mineralized (skull bones, vertebrae) and most bones are separated in space, which makes it difficult to 3D print. \textbf{Right:} many cylinders (more than one hundred) of different lengths and diameters were created in order to connect the different elements of this model: such a model is easier to print in 3D. }
