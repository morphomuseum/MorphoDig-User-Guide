...
\posttitle{
\\
  \includegraphics[width=8cm]{tutorial02.png}\\[\bigskipamount]
\end{center}}
...
A convenient way to orient an inner ear in a biologically relevant way is done as follows:\\
...
4) rotate the inner around the z axis ("rz" yellow slider) until the later semicircular points towards the top-left part of the screen, and the cochlea points towards the right-down part of the screen.\\
...
mouse (File -> Position-> Open Position for selected surfaces, then chose "Mouse\_right\_ear.pos", see Fig. \ref{orientation} p.\pageref{orientation}). If you plan to place landmarks
...
"Mouse\_right\_ear.ntw"). Once loaded, the mouse cranium surface file object is opened, is given the position
...
can load the enclosed .stv file (File->Landmarks->Open MorphoDig Landmark/Curve file (.STV)-> Mouse\_right\_ear.stv or drag and drop this file directly in the 3D main window), which contains 31 landmarks. You can change the way landmarks are rendered in the "Landmark and flag options" window
